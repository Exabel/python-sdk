...
class TestCsvTimeSeriesLoader(unittest.TestCase):
    def test_read_csv_should_failed_by_non_numeric_signal_values(self):
        ...
        expected_error_message_only_alphanumeric_characters = "".join(
            s for s in expected_error_message if s.isalnum()
        )
        actual_error_message_only_alphanumeric_characters = "".join(
            s for s in str(exception) if s.isalnum()
        )
        self.assertEqual(
            expected_error_message_only_alphanumeric_characters,
            actual_error_message_only_alphanumeric_characters,
        )
