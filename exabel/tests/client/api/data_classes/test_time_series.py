...
from exabel.client.api.data_classes.time_series import (
    Dimension,
    TimeSeries,
...
class TestTimeSeriesResourceName(unittest.TestCase):
    ...
    def test_time_series_resource_name(self) -> None:
        assert self.ts_name.entity_type == "entity_type"
        assert self.ts_name.entity == "entity"
        assert self.ts_name.signal == "signal"

    def test_entity_name(self) -> None:
        assert self.ts_name.entity_name == "entityTypes/entity_type/entities/entity"

    def test_signal_name(self) -> None:
        assert self.ts_name.signal_name == "signals/signal"

    def test_canonical_name(self) -> None:
        ts_name = TimeSeriesResourceName("entity_type", "entity", "signal")
        assert ts_name.canonical_name == "entityTypes/entity_type/entities/entity/signals/signal"

    def test_from_string(self) -> None:
        ts_name = TimeSeriesResourceName.from_string(
            "entityTypes/entity_type/entities/entity/signals/signal"
        )
        assert ts_name.entity_type == "entity_type"
        assert ts_name.entity == "entity"
        assert ts_name.signal == "signal"
...
class TestTimeSeries(unittest.TestCase):
    def test_proto_conversion(self):
        ...
        assert time_series.units == time_series_proto_converted.units
    ...
    def test_time_series_name(self):
        ...
        assert time_series.name == "entityTypes/country/entities/no/signals/customerA.revenue"
        assert (
...
class TestUnits(unittest.TestCase):
    def test_proto_conversion(self):
        ...
        assert units == Units.from_proto(units.to_proto())
