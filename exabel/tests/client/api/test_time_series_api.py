...
from exabel.client.api.data_classes.time_series import TimeSeries
...
from exabel.stubs.exabel.api.data.v1.all_pb2 import TimeSeries as ProtoTimeSeries
from exabel.util.import_ import estimate_size, get_batches_for_import
...
class TestTimeSeriesApi:
    ...
    def test_estimate_size_constants(self):
        ...
        assert 19 == point_without_known_time.ByteSize()
        ...
        assert 27 == point_with_known_time.ByteSize()
    ...
    def test_estimate_size_calculation(self):
        ...
        ).ByteSize() == estimate_size(series_with_known_time)
    ...
    def test_get_batches_for_import(self):
        ...
        assert 4619 == len(batches[0])
        assert 4619 == len(batches[1])
        assert 762 == len(batches[2])
        ...
        assert 1062370 == request.ByteSize()
    ...
    def test_get_signal_time_series_iterator(self):
        ...
        assert 1100 == len(ts)
        assert [f"ts_{i}" for i in range(1100)] == ts
