...
class TestResourceCreationResult(unittest.TestCase):
    def test_get_resource_name(self):
        ...
        assert get_resource_name(entity) == "entity_name"
        assert get_resource_name(relationship) == "relationship_type/from_entity/to_entity"
        assert get_resource_name(time_series) == "time_series_name"

    def test_resource_creation_result(self):
        entity = mock.Mock(Entity)
        entity.name = "entity_name"
        result = ResourceCreationResult(status=ResourceCreationStatus.FAILED, resource=entity)
        assert result.status == ResourceCreationStatus.FAILED
        assert result.resource == entity
        assert result.resource_name == "entity_name"
    ...
    def test_update(self):
        ...
        assert total + other_total == results.total_count
        assert total + other_total == len(results.results)
        assert 0.3 == results.abort_threshold
