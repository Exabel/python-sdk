...
from exabel.client.api.search_service import SearchService
...
from exabel.util.resource_name_normalization import (
    EntityResourceNames,
...
class TestTimeSeriesFileParser:
    def test_from_file__excel_with_batch_size_should_fail(self):
        ...
        assert "Cannot specify batch size when uploading Excel files." == str(cm.value)

    @mock.patch("exabel.services.file_time_series_parser.CsvReader.read_file")
    ...
    def test_parse_file__with_data_frame__with_header_should_fail(self):
        ...
        assert "Cannot specify header when uploading in batches." == str(cm.value)


class TestParsedTimeSeriesFile:
    ...
    def test_drop_duplicate_data_points__logs_warnings(self, caplog):
        series = pd.Series(
            [0, 1, 1],
            index=[0, 1, 1],
            name="time_series",
        )
        logger = "exabel.services.file_time_series_parser"
    ...
    def test_drop_time_series_with_duplicates_in_index(self, caplog):
    ...
    def test_validate_long_format_columns(self):
        assert SignalNamesInRows.is_valid(
            pd.DataFrame(columns=["mic:ticker", "signal", "value", "date"])
        )
        assert SignalNamesInRows.is_valid(
            pd.DataFrame(columns=["1brand", "signal", "value", "date"])
        )
        assert not SignalNamesInRows.is_valid(
            pd.DataFrame(columns=["mic:ticker", "signal", "brand", "value", "date"])
        )
        assert not SignalNamesInRows.is_valid(
            pd.DataFrame(columns=["mic:ticker", "signal", "value"])
        )

    def test_validate_medium_format_columns(self):
        assert SignalNamesInColumns.is_valid(
            pd.DataFrame(columns=["mic:ticker", "date", "signal1"])
        )
        assert not SignalNamesInColumns.is_valid(
            pd.DataFrame(columns=["mic:ticker", "date", "signal1", "1thing"])
        )
        assert SignalNamesInColumns.is_valid(pd.DataFrame(columns=["1thing", "date", "signal1"]))
        assert not SignalNamesInColumns.is_valid(
            pd.DataFrame(columns=["date", "signal1", "1thing"])
        )

    def test_remove_dot_int(self):
        assert "asdf.fefe" == _remove_dot_int("asdf.fefe")
        assert "asdf." == _remove_dot_int("asdf.")
        assert ".asdf" == _remove_dot_int(".asdf")
        assert "asdf" == _remove_dot_int("asdf.1")
        assert "asdf" == _remove_dot_int("asdf.10000")
        assert "asdf.1" == _remove_dot_int("asdf.1.100")
    ...
    def test_from_data_frame_long_and_medium_format(self):
        ...
        for parser in parsers:
            ...
            assert lookup_result.mapping == {
                k: f"entityTypes/company/entities/{v}" for k, v in BLOOMBERG_TICKER_MAPPING.items()
            }
            assert lookup_result.warnings == [
                EntitySearchResultWarning(
...
class TestMetaDataSignalNamesInRows:
    def test_get_deduplicated_series(self, caplog):
        series = [
            TimeSeries(
        ...
        assert [series[0]] == series_deduplicated
        assert series[2:] == series_with_duplicate_names
