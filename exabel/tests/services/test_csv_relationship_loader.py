...
from exabel.services.csv_loading_constants import (
    DEFAULT_NUMBER_OF_RETRIES,
    DEFAULT_NUMBER_OF_THREADS,
)
from exabel.services.csv_loading_result import CsvLoadingResult
from exabel.services.csv_relationship_loader import (
...
from exabel.stubs.exabel.api.data.v1.entity_messages_pb2 import Entity
from exabel.tests.client.exabel_mock_client import ExabelMockClient


class TestRelationshipLoaderColumnConfiguration:
    ...
    def test_from_default_values(self):
        ...
        assert config.from_column.entity_type is None
        assert 0 == config.from_column.index
        assert config.to_column.entity_type is None
        assert 1 == config.to_column.index
    ...
    def test_from_entity_types(self):
        ...
        assert "from_entity_type" == config.from_column.entity_type
        assert 0 == config.from_column.index
        assert "to_entity_type" == config.to_column.entity_type
        assert 1 == config.to_column.index
    ...
    def test_from_entity_types__with_entity_columns(self):
        ...
        assert "from_entity_type" == config.from_column.entity_type
        assert "from_entity_column" == config.from_column.name
        assert "to_entity_type" == config.to_column.entity_type
        assert "to_entity_column" == config.to_column.name
    ...
    def test_from_entity_types__with_identifiers(self):
        ...
        assert "company" == config.from_column.entity_type
        assert "isin" == config.from_column.identifier_type
        assert 0 == config.from_column.index
        assert "company" == config.to_column.entity_type
        assert "isin" == config.to_column.identifier_type
        assert 1 == config.to_column.index
...
class TestCsvRelationshipLoader(unittest.TestCase):
    def test_load_relationships_with_properties(self):
        ...
        CsvRelationshipLoader(client).load_relationships(
            filename="exabel/tests/resources/data/relationships_with_properties.csv",
    ...
    def test_load_relationships_with_non_existent_relationship_type(self):
        ...
        with pytest.raises(FileLoadingException):
            CsvRelationshipLoader(client).load_relationships(
                filename="exabel/tests/resources/data/relationships.csv",
    ...
    def test_load_relationships_with_existent_relationship_type_existent_uppercase_entity_type(
        ...
        self.assertCountEqual(expected_relationships, actual_relationships)
        assert isinstance(result, FileLoadingResult)
        assert isinstance(result, CsvLoadingResult)
    ...
    def test_load_relationships_only_entity_from_column_specified(self):
        client: ExabelClient = ExabelMockClient()
        with pytest.raises(FileLoadingException) as exception_context:
            CsvRelationshipLoader(client).load_relationships(
                filename="exabel/tests/resources/data/relationships.csv",
                relationship_type="NON_EXISTENT_RELATIONSHIPTYPE",
                from_entity_column="entity_from",
    ...
    def _check_load_with_entity_types(
        ...
        **csv_loader_kwargs,
        ...
        namespace = csv_loader_kwargs.pop("namespace", None)
        if namespace:
    ...
    @mock.patch("exabel.services.csv_relationship_loader.CsvReader")
