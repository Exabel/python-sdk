...
class TestCsvReader:
    ...
    def test_read_csv_in_chunks(self):
        content = [("Column A", "Column B"), ("a1", "a2"), ("b1", "b2")]
        result = self._read_csv(content, string_columns=["Column A", "Column B"], chunksize=1)
        expected = pd.DataFrame({"Column A": ["a1"], "Column B": ["a2"]})
        pd.testing.assert_frame_equal(expected, next(result))
        expected = pd.DataFrame({"Column A": ["b1"], "Column B": ["b2"]}, index=[1])
        pd.testing.assert_frame_equal(expected, next(result))
