...
class TestResourceNameNormalization:
    def test_basic(self):
        assert "abc_Inc_" == normalize_resource_name("abc, Inc.")
        assert "abcXYZ0189-_" == normalize_resource_name("abcXYZ0189-_")
        assert "_l_" == normalize_resource_name("-Øl?.")
        long_name = "".join(str(i) for i in range(50))
        assert long_name[:64] == normalize_resource_name(long_name)
    ...
    def test_validate_mic_ticker(self):
        assert _validate_mic_ticker("A:A")

        assert not _validate_mic_ticker(":A")
        assert not _validate_mic_ticker("A:")
        assert not _validate_mic_ticker("A:A:A")
        assert not _validate_mic_ticker("A::A")
        assert not _validate_mic_ticker("A")
    ...
    def test_isin_mapping(self):
        ...
        assert "entityTypes/company" == call_args_list[0][1]["entity_type"], (
            "Arguments not as expected"
        )
        assert list(search_terms) == list(call_args_list[0][1]["terms"])
        pd.testing.assert_series_equal(expected, result)
    ...
    def test_entity_mapping(self):
        ...
        assert not entity_api.search_for_entities.called
    ...
    def test_name_collision(self):
        ...
        with pytest.raises(ValueError):
            _assert_no_collision(bad_mapping)
    ...
    def test_get_namespace_from_entity_type_name(self):
        assert "ns" == get_namespace_from_resource_identifier("ns.entity_type")
        assert get_namespace_from_resource_identifier("entity_type") is None

    def test_get_namespace_from_entity_type_name__invalid_name_should_fail(self):
        with pytest.raises(ValueError):
            get_namespace_from_resource_identifier("ns.ns2.entity_type")
