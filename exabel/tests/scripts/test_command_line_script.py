...
class TestCommandLineScript(unittest.TestCase):
    ...
    def test_command_line_script(self):
        assert ["the-script-name", "--the-argument", "123"] == self.script.argv
        assert "The description." == self.script.parser.description

    def test_parse_arguments(self):
        args = self.script.parse_arguments()
        assert argparse.Namespace(the_argument="123") == args
