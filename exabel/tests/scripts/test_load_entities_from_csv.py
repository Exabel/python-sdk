...
from exabel.scripts.load_entities_from_csv import LoadEntitiesFromCsv
...
class TestLoadEntities:
    def test_read_file_with_uppercased_entity_type_in_column_non_existing(self):
        args = common_args + [
            "--filename",
            "./exabel/tests/resources/data/entities.csv",
            "--entity-column",
            "brand",
            "--display-name-column",
            "brand",
    ...
    def test_read_file__non_existent_entity_column_should_fail(self):
        ...
        assert "company" == context.value.args[0]
    ...
    def test_read_file_with_integer_identifier(self):
        file_args = common_args + [
            "--filename",
            "./exabel/tests/resources/data/entities_with_integer_identifiers.csv",
        ]
    ...
    def check_entities(
        ...
        assert sorted(expected_entities) == sorted(all_entities)
    ...
    def test_read_file_with_duplicated_entity_identifiers_should_fail(self):
        args = common_args + [
            "--filename",
            "./exabel/tests/resources/data/entities_with_duplicated_brands.csv",
            "--description-column",
            "description",
        ]
        with pytest.raises(ValueError) as context:
            load_test_data_from_csv(LoadEntitiesFromCsv, args)
        assert "Duplicate entities detected" == str(context.value)

    def test_read_file_random_errors(self):
        random.seed(1)
        args = common_args + [
            "--filename",
            "./exabel/tests/resources/data/entities2.csv",
        ]
        client = load_test_data_from_csv(LoadEntitiesFromCsv, args)
    ...
    @patch("exabel.scripts.load_entities_from_csv.parse_property_columns")
    def test_property_columns(self, mock_parser):
        ...
        client = MagicMock()
        with patch("exabel.scripts.load_entities_from_csv.CsvEntityLoader"):
