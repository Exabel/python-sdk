...
from exabel.util.resource_name_normalization import validate_signal_name
...
class TestUploadTimeSeries(unittest.TestCase):
    ...
    def test_get_series(self):
        ...
        assert 1 == len(time_series.failures)
        assert 1 == len(time_series.valid_series)
        pd.testing.assert_series_equal(
            pd.Series(
                [1, 2],
    ...
    def test_read_file_without_pit(self):
        args = common_args + [
            "--filename",
            "./exabel/tests/resources/data/timeseries.csv",
        ]
    ...
    def test_read_file_with_multiple_signals(self):
        args = common_args + [
            "--filename",
            "./exabel/tests/resources/data/timeseries_multiple_signals.csv",
            "--pit-offset",
            "0",
        ]
        script = LoadTimeSeriesFromFile(args)
    ...
    def test_read_file_in_long_format(self):
        args = common_args + [
            "--filename",
            "./exabel/tests/resources/data/timeseries_long_format.csv",
            "--pit-offset",
            "0",
        ]
        script = LoadTimeSeriesFromFile(args)
    ...
    def test_read_file_with_known_time(self):
        args = common_args + [
            "--filename",
            "./exabel/tests/resources/data/timeseries_known_time.csv",
        ]
        script = LoadTimeSeriesFromFile(args)
        script.run_script(self.client, script.parse_arguments())
    ...
    def test_read_file_with_integer_identifiers(self):
        args = common_args + [
            "--filename",
            "./exabel/tests/resources/data/timeseries_with_integer_identifiers.csv",
            "--pit-offset",
            "30",
        ]
    ...
    def test_read_file_with_nan_identifiers(self):
        args = common_args + [
            "--filename",
            "./exabel/tests/resources/data/timeseries_with_empty_identifiers.csv",
            "--pit-offset",
            "30",
        ]
        with pytest.raises(SystemExit):
    ...
    def test_should_fail_with_invalid_data_points(self):
        args = common_args + [
            "--filename",
            "./exabel/tests/resources/data/time_series_with_invalid_data_points.csv",
        ]
    ...
    def test_should_fail_with_missing_date(self):
        args = common_args + [
            "--filename",
            "./exabel/tests/resources/data/timeseries_with_missing_date.csv",
            "--pit-offset",
            "1",
        ]
    ...
    def test_should_fail_with_missing_known_time(self):
        args = common_args + [
            "--filename",
            "./exabel/tests/resources/data/timeseries_with_missing_known_time.csv",
        ]
    ...
    def test_valid_replace_existing_time_series(self):
        ...
        assert replace_existing_time_series_status
    ...
    def test_replace_existing_time_series_across_batches(self):
        ...
        call_args_list = self.client.time_series_api.bulk_upsert_time_series.call_args_list
        assert 4 == len(call_args_list)
        assert call_args_list[0][1]["replace_existing_time_series"]
        assert (
            "entityTypes/company/entities/company_A/signals/acme.signal1"
            == call_args_list[0][0][0][0].name
        )
        assert "replace_existing_time_series" not in call_args_list[1][1]
        assert (
            "entityTypes/company/entities/company_A/signals/acme.signal1"
            == call_args_list[1][0][0][0].name
        )
        assert call_args_list[2][1]["replace_existing_time_series"]
        assert (
            "entityTypes/company/entities/company_B/signals/acme.signal1"
            == call_args_list[2][0][0][0].name
        )
        assert "replace_existing_time_series" not in call_args_list[3][1]
        assert (
            "entityTypes/company/entities/company_B/signals/acme.signal1"
    ...
    def test_replace_existing_data_points_across_batches(self):
        ...
        call_args_list = self.client.time_series_api.bulk_upsert_time_series.call_args_list
        assert 3 == len(call_args_list)
        assert call_args_list[0][1]["replace_existing_data_points"]
        assert (
            "entityTypes/company/entities/company_A/signals/acme.signal1"
            == call_args_list[0][0][0][0].name
        )
        assert call_args_list[2][1]["replace_existing_data_points"]
        assert (
            "entityTypes/company/entities/company_B/signals/acme.signal1"
            == call_args_list[2][0][0][0].name
        )
    ...
    def test_load_time_series_with_duplicate_indexes_should_fail(self):
        with pytest.raises(FileLoadingException) as exc_info:
            self._partial_load_time_series()
        assert 1 == len(exc_info.value.failures)
        assert [1.0, 2.0] == exc_info.value.failures[0].resource.to_list()
    ...
    def test_load_time_series_with_uppercase_signals_not_existing(self):
        args = common_args + [
            "--filename",
            "./exabel/tests/resources/data/timeseries_with_uppercase_columns.csv",
            "--create-missing-signals",
        ]
        script = LoadTimeSeriesFromFile(args)
        ...
        assert 1 == len(call_args_list_update_signal)
        signal = call_args_list_update_signal[0][0][0]
        assert "signals/ns.signal1" == signal.name
    ...
    def test_load_time_series_with_uppercase_signals_existing(self):
        ...
        call_args_list = self.client.time_series_api.bulk_upsert_time_series.call_args_list
        assert 1 == len(call_args_list)
        assert 0 == len(self.client.signal_api.update_signal.call_args_list)
        series = call_args_list[0][0][0]
        assert 1 == len(series)
    ...
    def test_load_time_series_with_uppercase_signals_not_existing_case_sensitive(self):
        args = common_args + [
            "--filename",
            "./exabel/tests/resources/data/timeseries_with_mixedcase_columns.csv",
            "--create-missing-signals",
    ...
    def test_load_time_series_default_optimise(
        ...
        call_args_list = self.client.time_series_api.bulk_upsert_time_series.call_args_list
        assert call_args_list[0][1]["should_optimise"] is None
    ...
    def test_load_time_series_optimise(
        ...
        call_args_list = self.client.time_series_api.bulk_upsert_time_series.call_args_list
        assert call_args_list[0][1]["should_optimise"]
    ...
    def test_load_time_series_no_optimise(
        ...
        call_args_list = self.client.time_series_api.bulk_upsert_time_series.call_args_list
        assert not call_args_list[0][1]["should_optimise"]
